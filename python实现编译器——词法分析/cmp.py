...
class word_list():
    ...
    def creat_table(self, in_words):
        ...
        for word in in_words:
            ...
            # 判断是否为字符串
            if char_flag == True:
                self.word_list.append({'line':line, 'type':'chartable', 'word':w})
                continue
            # 判断为关键字
            if w in k_list:
            ...
            # 其他字符处理
                ...
                if if_num(w):
                    self.word_list.append({'line':line, 'type':'number', 'value':w})
                # 如果是变量名要判断是否已经存在
                elif if_name(w):
                    if have_name(self.name_list,w):
                        pass
                    else:
                        self.name_list.append({'line':line, 'id':name_id, 'value':0.0, 'name':w})
