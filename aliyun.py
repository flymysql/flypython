...
# 		文章插入，数据库部分
#################################################
def insert_post(ti,au,con,ta,tim,sum,db):
	...
	if data is None:
		...
		insert_post = "INSERT INTO `wp_posts` (`ID`, `post_author`, `post_date`, `post_date_gmt`, `post_content`, `post_title`, `post_excerpt`, `post_status`, `comment_status`, `ping_status`, `post_password`, `post_name`, `to_ping`, `pinged`, `post_modified`, `post_modified_gmt`, `post_content_filtered`, `post_parent`, `guid`, `menu_order`, `post_type`, `post_mime_type`, `comment_count`) VALUES (NULL, '" + str(author_id[0]) + "', '" + str(time) + "', '" + str(time) + "', '" + str(content) + "', '" + str(title) + "', '" + summ + "', 'publish', 'open', 'open', '', '', '', '', '" + str(time) + "', '" + str(time) + "', '', '0', '', '0', 'post', '', '0')"
		try:
...
def get_post(lin, tag):
	...
	try:
		...
		time.sleep(3)
		# get_url(nextlink)
	return insert_post(title,author,texts,tag,tim,str(summary),db)
